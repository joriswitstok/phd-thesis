...
\nomenclature[f-JWST]{\textit{JWST}}{\textit{James Webb\footnote{James Webb is credited for his unwavering support of space science while successfully executing the Apollo moon programme. His tenure as NASA administrator, however, coincided with the ``Lavender Scare'', a period during which the US government actively persecuted LGBTQ+ employees (although no evidence of his direct involvement has emerged; see \url{https://www.nasa.gov/feature/nasa-shares-james-webb-history-report}).} Space Telescope}}
...
\nomenclature[f-JADES]{JADES}{\textit{JWST} Advanced Deep Extragalactic Survey}
...
\else
    ...
    \backgroundsetup{
        ...
        placement=top,
        angle=0,
        contents={\includegraphics[width=\paperwidth]{Images/Tarantula_nebula1}}
    }
    \BgThispage
...
\else
    ...
    \backgroundsetup{
        ...
        placement=top,
        angle=0,
        contents={\includegraphics[width=\paperwidth]{Images/Tarantula_nebula2}}
    }
    \BgThispage
...
\begin{enumerate}[label=(\roman*)]
    \item the current expansion rate of the Universe, as measured through the Hubble constant,\footnote{As discussed in \cref{chIssec:Structure_formation_in_a_homogeneous_and_isotropic_universe}, there is significant tension between this measurement and values based on local-Universe probes, $H_0 = 73 \pm 1 \, \mathrm{km \, s^{-1} \, Mpc^{-1}}$ \citep[e.g.][]{2022ApJ...934L...7R}.} is $H_0 = 67.4 \pm 0.5 \, \mathrm{km \, s^{-1} \, Mpc^{-1}}$;
    \item dark matter significantly dominates over baryonic matter, which only makes up around one sixth of the matter content ($\Omega_{\text{b}, \, 0} = 0.0493 \pm 0.0002$, while $\Omega_{\text{m}, \, 0} = 0.315 \pm 0.007$);
    \item dark energy currently trumps matter ($\Omega_{\Lambda, \, 0} = 0.685 \pm 0.007$) and is consistent with a cosmological constant (i.e. equation of state $w = -1$);
...
Despite the success of the \LCDM\ model in reproducing the statistical properties of the CMB, several conceptual problems remain. How can a homogeneous and isotropic model universe, hypothetically perfectly adhering to the assumptions of the Cosmological Principle, ever achieve the observed complexity such as that shown in \cref{chIfig:SMACS}? This issue can be resolved by considering the model as an approximation that becomes valid at sufficiently early times and/or large scales, at the present day $l \gtrsim 200 \, \mathrm{cMpc}$ (where $\mathrm{cMpc}$ denotes a comoving megaparsec) according to simulations \citep[e.g.][]{2010MNRAS.405.2009Y}. Indeed, the GR description of a strictly homogeneous cosmological fluid does not see the formation of any structure; such a model universe devoid of galaxies, stars, and planets is evidently not a representative model for the observed Universe. This statement may seem obvious, but it underlines the effectiveness of the relatively simple \LCDM\ model at explaining the formation of complex cosmological structures as local deviations from a homogeneous background.
...
Another consequence of relativistic effects is a distinction between the growth of modes within and beyond the (comoving) particle horizon at a given time $t$,
\begin{equation}
...
\begin{equation}
    ...
    c_\text{s}^2 = \frac{5}{3} \frac{k_\text{B} T}{m_\text{p}} \, ,
\end{equation}
...
When baryons fall into the potential wells of dark matter halos that establish after recombination (\cref{chIsssec:Dark_matter_instabilities}), gas would, in the absence of cooling, be shock-heated to the virial temperature upon reaching the virial radius, establishing a hydrostatic equilibrium that prevents further collapse. Provided the cooling process is efficient, however, gas is able to contract further, though it retains its angular momentum since the emission is isotropic. In this case, a flattened, rotating, gaseous disc, fed by smooth accretion of cold gas, forms at the centre of the potential well.\footnote{For the most massive halos, $M \gtrsim 10^{11} \, \mathrm{M_\odot}$, cooling is generally inefficient (as discussed in \cref{chIsssec:Dissipative_processes}) and a stable shock front forms \citep[``hot-mode'' accretion;][]{2003MNRAS.345..349B}. However, hydrodynamical simulations reveal a more complex picture, where cold gas flows are still able to penetrate the hot halo to reach the central galaxy \citep{2020MNRAS.499..597B}.} Within the disc, gas is able to fragment further and is converted into stars (discussed in further detail in \cref{chIssec:Star_formation_and_stellar_evolution}).
...
Stars, in particular the hotter (O- and B-type) variants, can produce a substantial amount of continuum emission at wavelengths below the Lyman limit, also known as Lyman-continuum (LyC) radiation. Stars, or ionising sources in general, will therefore ionise hydrogen gas surrounding them, forming a so-called \HII\ region \citep{1939ApJ....89..526S}. This is discussed in detail in \cref{app:Photoionisation_model}, where several models of an \HII\ region of increasing complexity are considered. I introduce the most important concepts here, the first of which is the \citeauthor{1939ApJ....89..526S} radius $R_\text{S}$,
\begin{equation}
    \label{chIeq:Photoionisation_equilibrium}
    R_\text{S} = \left( \frac{3 \dot{N}_\text{ion}}{4 \pi \alpha_\text{B} \, n_\text{H}^2} \right)^\frac{1}{3} \, ,
\end{equation}

\noindent enclosing the volume $\frac{4}{3} \pi R_\text{S}^3$ for which ionisation equilibrium holds. Detailed calculations show that indeed hydrogen is ionised to a high degree within the \citeauthor{1939ApJ....89..526S} radius (\cref{app:Photoionisation_model}). A second important quantity is the ionisation parameter, ordinarily defined as
\begin{equation}
...
In answering these questions with observations we are mainly held back by our incomplete view of galaxies in the EoR: currently, we mostly probe the rest-frame UV, restricting us to the unobscured light that is biased towards the youngest stellar populations (cf. \cref{chIsssec:Counting_stars,chIssec:Stellar_continuum}; see also \citealt{2022arXiv220803281T}). However, we can already start to address several of these questions in modest samples of galaxies for example by exploring new rest-frame UV diagnostics and by studying the FIR emission, as will be done with the help of cosmological simulations in \cref{ch:Prospects_for_observing_the_low-density_cosmic_web_in_Lya_emission} and in light of observations with VLT, \textit{HST}, and ALMA in \cref{ch:Assessing_the_sources_of_reionisation,ch:Dual_constraints_with_ALMA}. Specifically, \cref{ch:Prospects_for_observing_the_low-density_cosmic_web_in_Lya_emission} revolves around questions related to question~(i), namely of how we can study the intergalactic gas reservoirs from which galaxies accrete, and, more broadly, how we can study the formation of structure in the Universe on the largest scales. In \cref{ch:Assessing_the_sources_of_reionisation}, I will address the questions~(ii), (iii), and (v) through the detailed study with the VLT of the radiation field, ionisation state, metal enrichment, and LyC escape in the ISM of a metal-poor galaxy at $z \sim 5$. \cref{ch:Dual_constraints_with_ALMA} considers questions~(ii), (iii), and (iv) by characterising the early build-up of metals and dust and other physical properties of the ISM in a sample of $z \sim 7$ galaxies observed with ALMA and \textit{HST}.
